...
class ExchangeService {
    ...
    static async getLiveRate(from, to) {
        ...
        // 3. Check Cache
        const now = Date.now();
        const isCacheExpired = !this.cache || now - this.cache.timestamp > this.CACHE_TTL_MS;
        if (isCacheExpired) {
        ...
        if (!rateFrom || !rateTo) {
            console.error(`❌ Missing rate data in cache for ${base} or ${target}`);
            console.log('Available Keys:', Object.keys(rates).slice(0, 5), '...');
    ...
    static async refreshRates() {
        try {
            console.log('📡 API Call: https://api.frankfurter.app/latest?from=USD');
            ...
            console.log(`💾 Cache updated with ${Object.keys(data.rates).length} currencies.`);
        }
        catch (error) {
